angular.module('mrlapp.service.IntroGui', []).controller('IntroGuiCtrl', ['$scope', 'mrl', '$timeout', function($scope, mrl, $timeout) {
    ...
    $scope.introActionsOpen = false
    var servicePanels = ['runtime', 'python', 'security', 'opencv', 'arduino', 'servo01', 'servo02']
    $scope.getIntroServiceName = function(panelName) {
        if (!panelName || !$scope.subPanels[panelName]) return null
        return $scope.subPanels[panelName].name
    }
    $scope.introDoRelease = function() {
        var panel = $scope.subPanels[$scope.activePanel]
        if (panel) mrl.sendTo('runtime', 'release', panel.name)
    }
    $scope.introDoSave = function() {
        var panel = $scope.subPanels[$scope.activePanel]
        if (panel) mrl.sendTo('runtime', 'save', panel.displayName)
    }
    $scope.introDoApply = function() {
        var panel = $scope.subPanels[$scope.activePanel]
        if (panel) {
            mrl.sendTo(panel.displayName, 'apply')
            mrl.sendTo(panel.displayName, 'broadcastState')
        }
    }
